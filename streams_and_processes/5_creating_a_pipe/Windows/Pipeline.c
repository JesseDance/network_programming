...
int main( )
{
   ...
   if (! CreatePipe(&hPipeOut_Rd, &hPipeIn_Wr, NULL, 0))
   {
      printError( "CreatePipe()" );
   }


   /* In order for child_1 to use the pipe's write handle,
      that handle must be inheritable by child_1.
      Note that child_1 does not need the pipe's read handle,
      so we do not (yet) make that handle inheritable. */
   if ( ! SetHandleInformation(hPipeIn_Wr, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT) )
   {
   ...
   /* The parent uses the child's STARTUPINFO data structure to
      ...
      pipe's input handle as inheritable. */
   ...
   /* child_2 does not need the pipe's write handle, so child_2
      should not inherit that handle. Since child_1 has already
      been created, we can close this no lomger needed handle.
      ...
      ever seeing an end-of-file condition (try it!). */
   ...
   /* In order for child_2 to use the pipe's read handle,
      that handle must be inheritable by child_2.  */
   if ( ! SetHandleInformation(hPipeOut_Rd, HANDLE_FLAG_INHERIT, HANDLE_FLAG_INHERIT) )
   {
      printError("SetHandleInformation(hPipeOut_Rd)");
   }
   ...
   /* The parent uses the child's STARTUPINFO data structure to
      ...
      The redirection would not work if we had not set the
      pipe's output handle as inheritable. */
   ...
   /* Now that child_2 has been created, we
      should close the pipe's output handle. */
