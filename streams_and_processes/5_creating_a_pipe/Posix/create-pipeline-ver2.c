/*
   ...
   is waiting) on the right end of the pipeline.
*/
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
...
int main()
{
   ...
   else  /* child 1 */
   {
      ...
      else if ( fork_rv > 0 ) /* parent */
      {
         ...
         /* Step 5: child 1 calls close(4), close(0), dup(3), close(3) */
         close(4);
         close(0);
         dup(3);
         close(3);
      ...
      else /* child 2 */
      {
         ...
         /* Step 6: child 2 calls close(3), close(1), dup(4), close(4) */
         close(3);
         close(1);
         dup(4);
         close(4);
