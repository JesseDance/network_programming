/*
    This program launches nslookup.exe, a console program.
    This program demonstrates a very strange situation
    where two console programs "fight" over the command-line.

    Open a cmd command window in the current folder and run
    this program from the command prompt (don't run this
    program by double clicking on its exe file).
       > CreateProcess2.exe
    Then the cmd.exe process and the nslookup.exe process will
    both end up simultaneously using the command-line of the
    console window!
*/
#include <windows.h>
#include <stdio.h>
...
int main( )
{
   ...
   {
      printf("Started program nslookup with pid = %d\n\n", (int)processInfo.dwProcessId);
   }
