/*
    ...
    Notice that the child process inherits its stdin, stdout, and
    stderr streams from this parent and it also inherits its
...
int main( int argc, char *argv[])
{
   ...
   /* Set some environment variables that the child can inherit. */
   _putenv( "a=first" );
   _putenv( "b=second" );
   _putenv( "c=third" );

   /* set up the command line (with several command-line arguments) */
   lpCommandLine = "environment-variables.exe a b c d";
   ...
   /* Remove the environment variables that we set. */
   _putenv( "a=" );
   _putenv( "b=" );
   _putenv( "c=" );
   _putenv( "d=this-is-a-test" );

   /* close all the handles */
   CloseHandle(processInfo.hThread);
   CloseHandle(processInfo.hProcess);

 //Sleep(60000); /* use this to examine this program's environment variables */
