/*
   ...
   This version uses the "open-close-dup-close" technique to
   redirect standard input. This uses the fundamental Linux idea
   that when you open() a file, the operating system always returns
  ...
  data.txt--->> 3             |
   ...
   A call to dup(3) then results in this because dup() will copy the
   given file descriptor to the lowest available file descriptor
   number (which is 0).
